...
<% 
	...
	// Logger logger = Logger.getLogger("com.olympus.assetupload.excel.servlet");
...
/*************************************************************************************************************************************************************/
	public ArrayList<String> createArrFromHashMap(JspWriter out, List<Map<String, String>> rowMap, Logger logger) throws IOException {
		...
		logger.info(dtf.format(now) + ": " + "------------------ Begin HashMap process: ");
		int i = 0;
		...
		now = LocalDateTime.now();
		logger.info(dtf.format(now) + ": " + "------------------End HashMap process: ");
		return (dataArr);
	}

	/*************************************************************************************************************************************************************/
	public String buildCells(JspWriter out, ArrayList<String> dataArr, Logger logger) throws IOException {
		DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss.SSS");
		LocalDateTime now = null;
		String cells = "";
		...
		now = LocalDateTime.now();
		logger.info(dtf.format(now) + ": " + "------------------ !!! Begin BuildCells process: ");
		
		System.out.println("*** SZ=" + dataArr.size() + "--");
		if (arrSZ > 0) {
		...
		now = LocalDateTime.now();
		logger.info(dtf.format(now) + ": " + "------------------ !!! End BuildCells process: ");
		return cells;
	}
	...
	public String buildCells_OLD(JspWriter out, ArrayList<String> dataArr, Logger logger) throws IOException {
		String cells = "";
		return cells;
	}%>
...
-->			
		...
		list = createArrFromHashMap(out, rowMap, logger);
	 	String cells =  buildCells(out, list, logger); // build data cells from file
 		 out.println(cells);
